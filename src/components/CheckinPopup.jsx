// src/components/CheckinPopup.jsx - REAL SESSION FIX
...
export default function CheckinPopup(){
  ...
  useEffect(()=>{
    async function check(){
      ...
      const {data} = await supabase.from('task_submissions').select('id').eq('user_id', user.id).eq('day_name','Daily').gte('created_at', todayISO).maybeSingle()
  ...
  const doCheckin = async ()=>{
    ...
    setTimeout(()=>{ setShow(false); alert(`🔥 Day ${newStreak} Checked for ${user?.email}! +${earn} KSH`) }, 800)
  }
  ...
  return <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.75)', backdropFilter:'blur(8px)', zIndex:50, display:'flex', alignItems:'center', justifyContent:'center', padding:16, boxSizing:'border-box'}}>
    <div style={{background:'#111', color:'white', border:'1px solid #222', padding:22, borderRadius:20, width:'100%', maxWidth:330, textAlign:'center', boxSizing:'border-box', animation:'pop 0.3s ease'}}>
      ...
      <p style={{margin:'0 0 6px', fontSize:12, color:'#888'}}>Miss a day? Streak resets to Day 1!</p>
      <p style={{margin:'0 0 14px', fontSize:10, color:'#0f7f8a'}}>{user?.email}</p>
      ...
      <button onClick={doCheckin} disabled={claimed} style={{width:'100%', padding:13, background: claimed? '#333' : 'linear-gradient(90deg, #0f7f8a 0%, #6a3db5 100%)', color:'white', border:'none', borderRadius:12, fontWeight:'bold', fontSize:14, cursor:'pointer'}}>
